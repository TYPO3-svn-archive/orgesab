...
/**
 * Class "tx_orgesab_QuotaTask" provides procedures for check quota and control orgesab mailboxes
 *
...
* @since         0.0.1
 */
class tx_orgesab_QuotaTask extends tx_scheduler_Task {
...
//t3lib_div::devlog( '[tx_orgesab_QuotaTask] ' . $prompt, $this->extKey, 0 );
  ...
  private function initMailboxesData( )
  {
    ...
    $select_fields  = "
                      ...
                      ";
    $from_table     = "fe_users";
    $where_clause   = "(tx_orgesab_homedir != '' OR tx_orgesab_maildir != '') AND deleted = 0";
    $groupBy        = null;
  ...
  private function initDRS( )
  {
    ...
    switch( $this->extConf['debuggingDrs'] )
    {
      ...
      default:
        ...
        t3lib_div::devlog( '[tx_orgesab_QuotaTask] ' . $prompt, $this->extKey, 3 );
        break;
    }
  }
 ...
 */
  private function drs_mailToAdmin( $subject, $body )
  {
    ...
    try 
    {
      ...
      $mailer->setFrom( array( $this->orgesab_orgesabAdminEmail => 'ORGESAB QUOTA' ) );
      $mailer->setReplyTo( array( $this->orgesab_orgesabAdminEmail => 'ORGESAB QUOTA' ) );
      $mailer->setSubject( 'ORGESAB QUOTA: ' . $subject );
    ...
    if( $this->drsModeQuotaTask || $this->drsModeQuotaError )
    {
      switch( $success )
      {
        ...
        default:
          ...
          t3lib_div::devLog( '[tx_orgesab_QuotaTask]: ' . $prompt, $this->extKey, -1 );
          break;
      }
    }
  }
  ...
  private function quotaRemove( )
  {
    ...
    switch( $this->orgesab_quotaMode )
    {
      ...
      default:
        ...
        if( $this->drsModeError )
        {
          $prompt = 'Quota mode is undefined: "' .  $this->orgesab_quotaMode . '"';
          t3lib_div::devLog( '[tx_orgesab_QuotaTask]: ' . $prompt, $this->extKey, 3 );
        }
          // DRS
    ...
    if( $this->drsModeQuotaTask )
    {
      $prompt = $this->mailboxData['pathToMailbox'] . '  overrruns the quota limit. ' .
                ...
                '';
      t3lib_div::devLog( '[tx_orgesab_QuotaTask]: ' . $prompt, $this->extKey, 2 );
    }
      // DRS
  ...
  {
    $quotaLimitInMegabyte = ( int ) $this->mailboxData['tx_orgesab_quota'];
    if( empty ( $quotaLimitInMegabyte ) )
    {
      $quotaLimitInMegabyte = ( int ) $this->orgesab_quotaLimitDefault;
    }
    ...
    switch( true )
    {
      case( $quotaLimitInMegabyte < 1 ):
        ...
        $prompt_03 = 'Org +ESAB Quota will die!';
        if( $this->drsModeError )
        {
          t3lib_div::devLog( '[tx_orgesab_QuotaTask]: ' . $prompt_01, $this->extKey, 3 );
          t3lib_div::devLog( '[tx_orgesab_QuotaTask]: ' . $prompt_02, $this->extKey, 2 );
          t3lib_div::devLog( '[tx_orgesab_QuotaTask]: ' . $prompt_03, $this->extKey, 3 );
        }
          // DRS
...
//    }
  ...
  private function quotaWarning( )
  {
    ...
    $quotaLimitWarnInBytes = $this->quotaLimitInBytes / 100 * $this->orgesab_quotaLimitWarn;
...
//      $prompt = 'Quota limit in bytes: ' . $this->quotaLimitInBytes;
//      t3lib_div::devLog( '[tx_orgesab_QuotaTask]: ' . $prompt, $this->extKey, 0 );
//      $prompt = 'Quota limit warn in per cent: ' . $this->orgesab_quotaLimitWarn;
//      t3lib_div::devLog( '[tx_orgesab_QuotaTask]: ' . $prompt, $this->extKey, 0 );
      $prompt = $this->mailboxData['pathToMailbox'] . '  overrruns the warning limit. ' .
                'Mailbox size is ' . $this->mailboxSizeInBytes . ' bytes. ' .
                'Warning limit is ' . ( $this->quotaLimitInBytes / 100 * $this->orgesab_quotaLimitWarn ) . '.' .
                '';
      t3lib_div::devLog( '[tx_orgesab_QuotaTask]: ' . $prompt, $this->extKey, 2 );
    }
      // DRS
  ...
  private function sendMailWarningQuotaIsNotOverrun( )
  {
    ...
    $reducedMailboxInMegabytes  = ( int ) ( $quotaLimitInMegabytes / 100 * $this->orgesab_quotaReduceMailbox );
    ...
    $marker['###ORGESABADMINCOMPANY###']        = $this->orgesab_orgesabAdminCompany;
    $marker['###ORGESABADMINNAME###']           = $this->orgesab_orgesabAdminName;
    $marker['###ORGESABADMINPHONE###']          = $this->orgesab_orgesabAdminPhone;
    $marker['###QUOTALIMITINMEGABYTES###']      = $quotaLimitInMegabytes;
    $marker['###REDUCEDMAILBOXINPERCENT###']    = $this->orgesab_quotaReduceMailbox;
    ...
    {
      case( 'warn' ):
        $body     = $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:label.copy' ) .
                    ': ' .
                    $this->orgesab_orgesabAdminName . PHP_EOL .
                    PHP_EOL .
                    $body;
        $to       = $this->mailboxData['email'];
        $cc       = $this->orgesab_orgesabAdminEmail;
        break;
      case( 'test' ):
        ...
        $body     = '[TEST] ' . PHP_EOL . 
                    ...
                    $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:label.copy' ) .
                    ': ' .
                    $this->orgesab_orgesabAdminName . PHP_EOL .
                    PHP_EOL .
                    $body;
      ...
      case( 'remove' ):
        ...
        if( $this->drsModeError )
        {
          $prompt = 'Quota mode is not allowed: "' .  $this->orgesab_quotaMode . '"';
          t3lib_div::devLog( '[tx_orgesab_QuotaTask]: ' . $prompt, $this->extKey, 3 );
        }
          // DRS
        return false;
  ...
  private function sendMailWarningQuotaIsOverrun( )
  {
    ...
    $subject  = $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:email.warn.overrunQuotaLimit.subject' );
    $body     = $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:email.warn.overrunQuotaLimit.body' );
    foreach( $marker as $key => $value )
    {
      $subject  = str_replace($key, $value, $subject );
      $body     = str_replace($key, $value, $body );
  ...
  {    
    $orgesabAdminEmail  = 'Admin' . 
                          ...
                          '. ';
// Kein Effekt
//    $quotaMode          = htmlspecialchars_decode( $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:label.quotaMode' ) ) .
//                          ': ' . 
//                          htmlspecialchars_decode( $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:label.quotaMode.' . $this->orgesab_quotaMode ) );
// &ouml; wird Ä   
//    $quotaMode          = html_entity_decode( $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:label.quotaMode' ) ) .
//                          ': ' . 
//                          html_entity_decode( $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:label.quotaMode.' . $this->orgesab_quotaMode ) );
    $quotaMode          = $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:label.quotaMode' ) .
                          ': ' . 
                          $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:label.quotaMode.' . $this->orgesab_quotaMode ) .
                          '. ';
    $quotaLimitDefault   = $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:label.quotaLimitDefault' ) .
                          ': ' . 
                          $this->orgesab_quotaLimitDefault . 
                          '. ';
    $quotaLimitRemove   = $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:label.quotaLimitRemove' ) .
                          ': ' . 
                          $this->orgesab_quotaLimitRemove .
                          '. ';
    $quotaLimitWarn     = $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:label.quotaLimitWarn' ) .
                          ': ' . 
                          $this->orgesab_quotaLimitWarn .
                          '. ';
    $quotaReduceMailbox   = $GLOBALS['LANG']->sL( 'LLL:EXT:orgesab/lib/scheduler/locallang.xml:label.quotaReduceMailbox' ) .
                          ': ' . 
                          $this->orgesab_quotaReduceMailbox .
                          '. ';
    return $quotaMode . $orgesabAdminEmail. $quotaLimitDefault . $quotaLimitRemove . $quotaLimitWarn . $quotaReduceMailbox;
  }
}

if (defined('TYPO3_MODE') && isset($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/orgesab/lib/scheduler/class.tx_orgesab_quotatask.php'])) {
  include_once($GLOBALS['TYPO3_CONF_VARS'][TYPO3_MODE]['XCLASS']['ext/orgesab/lib/scheduler/class.tx_orgesab_quotatask.php']);
}
